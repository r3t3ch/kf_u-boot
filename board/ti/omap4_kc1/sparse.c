...
int _unsparse_process_chunk(unsigned char **source, u64 *section_size)
{
	...
	if(state.chunk_offset >= sizeof(chunk_header_t)) {
		...
		case CHUNK_TYPE_RAW:
			if(*section_size > 0) {
				struct mmc *mmc = find_mmc_device(state.slot_no);
				if (!mmc) {
				...
				unsigned int prev_rem_data = chunk_cur_size % mmc->write_bl_len;
				unsigned int missing_bytes = prev_rem_data ? mmc->write_bl_len - prev_rem_data : 0;
				...
				if (missing_bytes > 0) {
					...
					if (mmc->block_dev.block_write(mmc->block_dev.dev,
						state.data, state.sector, 1) == 0)
					{
						printf(" FAILED!\n");
						lcd_printf("...failed\n\n");
					}
				...
				unsigned int copy_len = (chunk_avl_size - missing_bytes) / mmc->write_bl_len;
				unsigned int rem_data = chunk_avl_size - (copy_len * mmc->write_bl_len) - missing_bytes;

				if (mmc->block_dev.block_write(mmc->block_dev.dev,
					*source, state.sector, copy_len) == 0)
				{
					printf(" FAILED!\n");
					lcd_printf("...failed\n\n");
				}
		...
		case CHUNK_TYPE_DONT_CARE:
			if (state.last_header.total_sz != sizeof(chunk_header_t)) {
				...
				printf("sparse: skip 0x%08lx bytes\n", skip);
...
int _unsparse(unsigned char *source, u64 section_size)
{
	...
	while((state.remaining_chunks > 0) && (section_size>0) && (ret == 0)) {
		ret = _unsparse_process_chunk(&source, &section_size);
	}

	printf("sparse: remaining chunks: %u\n", state.remaining_chunks);
	return ret;
}

u8 do_unsparse_process(unsigned char *source, u64 section_size)
{
	if (_unsparse(source, section_size))
		return 1;
	return 0;
}
