...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if ((2 ==val)||(6 ==val))
    {
        ...
        setenv("bootargs",CONFIG_DIAGNOSTIC_512M_BOOTARGS);
#else
        setenv("bootargs",CONFIG_DIAGNOSTIC_BOOTARGS);
#endif
    }
#endif
	s = getenv ("verify");
...
/*****************************************
 * Routine: sdram_density
 * Description: Check DDR density
 * Return: density
 *****************************************/
unsigned int sdram_density(void)
{
        int ddr_density;

        __raw_writel(8, 0x4C000000 + 0x00000050);
        ddr_density =__raw_readb(0x4C000000 + 0x00000040);
...
#define IMAGE_COARSE_OFFSET            0
#define IMAGE_FINE_OFFSET              0

#else // define CONFIG_OTTER2
...
int do_booti (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	#if defined(CONFIG_OTTER)
	mem_len = sprintf(boot_str, " mem=%s","512M");
	#else
	mem_len = sprintf(boot_str, " mem=%s","1G");
	#endif
	if(sizeof(hdr->cmdline) >= (boot_len + mem_len + strlen(hdr->cmdline) + 1))
