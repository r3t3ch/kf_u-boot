...
#define CONFIG_USERDATA_IDME_ADDR 0xCA80000
...
static int get_userdata_idme_addr(unsigned int *addr)
{
        fastboot_ptentry *part = fastboot_flash_find_ptn(IDME_PART_NAME);
        if (part) {
                *addr = part->start;
                return 0;
        } else {
...
static int idme_read(unsigned char *pbuf, int is_boot)
{
        ...
        if (get_userdata_idme_addr(&block_addr)) {
            return -1;
        }
        ...
        if (is_boot){
                ...
                block_addr = IDME_OFFSET / CONFIG_MMC_BLOCK_SIZE;
        }
...
int idme_get_var(const char *name, char *buf, int buflen)
{
...

	{
		...
		block = (nv->offset + IDME_BASE_ADDR + IDME_OFFSET) / CONFIG_MMC_BLOCK_SIZE;     // get the sector address
...
/*
  ...
  6 : RMA
  7 : changing bootmode to 1 then shutdown
*/
unsigned long idme_get_bootmode(void)
{
	unsigned char buffer[CONFIG_IDME_SIZE];
	unsigned char *pidme_data;
	char bootmode[BM_SIZE+1];
	...
	// get the bootmode
	memcpy(bootmode, (pidme_data+GET_OFFSET(BM)), GET_SIZE(BM));
	bootmode[GET_SIZE(BM)+1] = 0;
	if (bootmode[0] == '7'){
		memcpy((pidme_data+GET_OFFSET(BM)), "1", GET_SIZE(BM));	
		idme_write(buffer, BOOT_PARTITION); /* write to boot partition */
		printf("System shutdown for bootmode:%s\n",bootmode);
		twl6030_shutdown();
	}
	...
	return simple_strtoul(bootmode, NULL, 10);
}
...
int idme_update_var(const char *name, const char *value)
{
	...
	block = (nv->offset + IDME_BASE_ADDR + IDME_OFFSET) / CONFIG_MMC_BLOCK_SIZE;
...
int idme_get_board_type_rev_string(char *bt)
{
        ...
        if ((pBd) && (pBd[0] != '\0') && bt){
                if (!strncmp(pBd, "000", 3)){
                        ...
                        bt += strlen("Radley-");
                }else if (!strncmp(pBd, "805", 3)){
                        strncpy(bt, "Otter2-Dummy", strlen("Otter2-Dummy-"));
                        bt += strlen("Otter2-Dummy-");
                }else if (!strncmp(pBd, "806", 3)){
                        strncpy(bt, "Otter2-Eng-", strlen("Otter2-Eng-"));
                        bt += strlen("Otter2-Eng-");
                }else if (!strncmp(pBd, "807", 3)){
                        strncpy(bt, "Otter2-Prod-", strlen("Otter2-Prod-"));
                        bt += strlen("Otter2-Prod-");
                }else {
                        goto unknown_board;
                }
                strncpy(bt, &pBd[3], 2);
