...
int fastboot_getvar(const char *rx_buffer, char *tx_buffer)
{
	...
	} else if(!strcmp(rx_buffer, "max-download-size")) {
		if (fastboot_interface->transfer_buffer_size)
			sprintf(all_response + strlen(all_response), "%lu", fastboot_interface->transfer_buffer_size);
...
extern int idme_get_board_type_rev_string(char *bt);
static char board_type[16];
...
int fastboot_init(struct cmd_fastboot_interface *interface) 
{
    ...
    twl6030_init_vusb();
    __raw_writel(0x101, 0x4A0093E0);    //disable ocp2scp_usb_phy_ick
    __raw_writel(0x100, 0x4A008640);    //disable ocp2scp_usb_phy_phy_48m
    for (ms=0;ms<100;ms++)
        udelay(100);//10ms
    __raw_writel(~(1), 0x4A002300);          //power up the usb phy

    memset(board_type, 0, sizeof(board_type));
    idme_get_board_type_rev_string(board_type);
	...
	device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "Zoom";
        ...
        printf("IDME board_type = %s\n", board_type);
        device_strings[DEVICE_STRING_PRODUCT_INDEX] = board_type;
