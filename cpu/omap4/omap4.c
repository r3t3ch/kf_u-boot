...
int misc_init_r(void)
{
	...
	if (smb347_init(&wall_charger))
		printf("WARNING: SMB347 init failed\n");

#if 0 /* Check battery condition even though wall charger is used.
	if (wall_charger == 1)
		/* Wall charger, we can directly boot into system */
		goto done;
#endif
#endif
	...
	if (clk_index) {
		printf("WARNING: MPU_DPLL is reconfigured\n");
		configure_mpu_dpll(clk_index - 1);
	}
	Init_LCD();
	Show_Logo();
	//Turn on Brightness to the normal one
	Set_Brightness(1);
#endif
	return 0;
}
...
{
	__raw_writel((1<<5) | (0 << 2), WD2_BASE + WCLR);
...
/****************************************
 * Routine: watchdog_start
 * Description: Start MPU watchdog
 *****************************************/
void watchdog_start(int sec)
{
	static u32 pattern = 0x1234;

	watchdog_init();

	printf("omap: mpu: watchdog: set timeout=%d sec\n", sec);
	__raw_writel(GET_WLDR_VAL(sec), WD2_BASE + WLDR);
	wait_for_command_complete(WD2_BASE);

	/* trigger */
	__raw_writel(pattern, WD2_BASE + WTGR);
	pattern = ~pattern;
	wait_for_command_complete(WD2_BASE);
